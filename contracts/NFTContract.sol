...
contract TRC721Metadata is Context, TRC165, TRC721 {
    // Token name
...
contract TRC721Enumerable is Context, TRC165, TRC721, ITRC721Enumerable {
        ...
        private
    {
        ...
        delete _ownedTokens[from][_ownedTokens[from].length];
...
contract NFT is TRC721Enumerable {
    enum rarity {
        earlyBird,
        sbt,
        transferSafe
    ...
    mapping(uint256 => uint256) ticketRates;
    mapping(uint256 => rarity) tokenRarity;
    address BURNER;
    address manager; // for royalties
    uint256 total_cap;
    // Token name
    string private _name;
    ...
    function mint(address user, rarity _rarity) external payable {
        require(_tokenIds.current() <= total_cap, "All tickets are minted");
        if (_rarity == rarity.earlyBird) {
            require(
                msg.value >= ticketRates[uint(_rarity)],
                "ticket rate is not enough!"
            );
            _mint(user, _tokenIds.current());
            tokenRarity[_tokenIds.current()] = _rarity;
            (bool success, ) = manager.call{value: msg.value}("");
            require(success, "mint early-bird::payment failed");
            _tokenIds.increment();
        } else if (_rarity == rarity.sbt) {
            _mint(user, _tokenIds.current());
            tokenRarity[_tokenIds.current()] = _rarity;
            _tokenIds.increment();
        } else {
    ...
    function tokenOwners(address user)
        external
        view
    ...
    function setTicketRates(rarity _rarity, uint256 _price) external {
        require(msg.sender == owner, "Only owner!");
        ticketRates[uint(_rarity)] = _price;
    }
    ...
    function safeTransferFrom(
        ...
        require(tokenRarity[tokenId] != rarity.sbt, "Non transferrable !");
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "TRC721: transfer caller is not owner nor approved"
        );
    ...
    function burnAndValidate(address _owner, uint256 _tokenId) external {
        require(msg.sender == address(BURNER), "Only burner wallet!");
