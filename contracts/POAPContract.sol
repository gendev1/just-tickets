...
contract POAP is TRC721Enumerable {
    address owner;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address manager; // for providing POAPs
    uint256 total_cap;
    // Token name
    string private _name;
    ...
    function mintPOAP(address user) external returns (uint256 tokenID) {
        require(_tokenIds.current() <= total_cap, "All tickets are minted");
        require(msg.sender == address(owner), "Only burner can issue POAP!");
